...
for arg in "$@"; do
  case "${arg}" in
    '--help' ) echo "$0 clone_name src_rclone_remote src_bucket pubsub_name_pattern dst_rclone_remote dst_bucket local_dir parallel access [include_pattern] [exclude_pattern]"; exit 0;;
  esac
done
...
for pubsub_name in `rclone --stats 0 --timeout 1m --log-level ERROR --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}.log lsf --max-depth 1 ${src_rclone_remote}:${src_bucket}/4PubSub | grep ${pubsub_name_pattern} | grep -v '^ *$' | sort -u`; do
  if test ! -d ${local_dir}/${access}/4PubSub/${pubsub_name}; then
    ...
    rclone --ignore-checksum --ignore-existing --no-gzip-encoding --no-traverse --no-update-modtime --size-only --stats 0 --timeout 1m --transfers ${parallel} --log-level ERROR --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}.log copy ${src_rclone_remote}:${src_bucket}/4PubSub/${pubsub_name}/${latest_created} ${local_dir}/${access}/4PubSub/${pubsub_name}
  fi
  local_latest_created=`ls -1 ${local_dir}/${access}/4PubSub/${pubsub_name} | grep -v ^.*\.tmp$ | tail -1`
  for newly_created in `rclone --stats 0 --timeout 1m --log-level ERROR --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}.log lsf --max-depth 1 ${src_rclone_remote}:${src_bucket}/4PubSub/${pubsub_name} | grep ${local_latest_created} -A 100000 | sed -e '1d' | grep -v '^ *$' | sort -u`; do
    unclone_num=1
    while test ${unclone_num} -ne 0; do
      now=`date -u "+%Y%m%d%H%M%S"`
      rclone --ignore-checksum --ignore-existing --no-gzip-encoding --no-traverse --no-update-modtime --size-only --stats 0 --timeout 1m --transfers ${parallel} --log-level DEBUG --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}_${now}_index_sub.log copyto ${src_rclone_remote}:${src_bucket}/4PubSub/${pubsub_name}/${newly_created} ${local_dir}/${access}/4PubSub/${pubsub_name}/${newly_created}.tmp
    ...
    while test ${unclone_num} -ne 0; do
      now=`date -u "+%Y%m%d%H%M%S"`
      rclone --ignore-checksum --ignore-existing --no-gzip-encoding --no-traverse --no-update-modtime --size-only --stats 0 --timeout 1m --transfers ${parallel} --log-level DEBUG --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}_${now}.log copy --files-from-raw ${local_dir}/${access}/4PubSub/${pubsub_name}/${newly_created}.tmp ${src_rclone_remote}:${src_bucket} ${dst_rclone_remote}:${dst_bucket}
    ...
    while test ${unclone_num} -ne 0; do
      now=`date -u "+%Y%m%d%H%M%S"`
      rclone --ignore-checksum --ignore-existing --no-gzip-encoding --no-traverse --no-update-modtime --size-only --stats 0 --timeout 1m --transfers ${parallel} --log-level DEBUG --log-file ${local_dir}/${access}/4Clone_log/${clone_name}/${clone_datetime}_${now}_index_pub.log copyto ${local_dir}/${access}/4PubSub/${pubsub_name}/${newly_created} ${dst_rclone_remote}:${dst_bucket}/4PubSub/${pubsub_name}/${newly_created}
