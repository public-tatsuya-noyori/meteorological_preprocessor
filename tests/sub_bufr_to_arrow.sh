      ...
      if test -s ${source_work_directory}/filtered_newly_created_file.tmp; then
        if test ${index_only} -eq 0; then
          ...
          if test -s ${work_directory}/processed_file.txt; then
            ...
            ./met_pre_bufr_to_arrow.py --debug RJTD ${local_work_directory}/sub_bufr_to_arrow.txt ${local_work_directory} 1>>${local_work_directory}/sub_bufr_to_arrow_stdout.log 2>>${local_work_directory}/sub_bufr_to_arrow_stderr.log
          fi
        else
