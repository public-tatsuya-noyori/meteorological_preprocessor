...
if test ${err_num} -eq 0; then
    ...
    if test ${is_pre} -eq 1 -a -s wis_jma_updated.txt; then
        updated_num=`cat wis_jma_updated.txt | wc -l`
        mkdir -p cache
        rm -rf wis_jma
        rm -f wis_jma_get2.txt
	rm -f wis_jma_to_cache.txt
	    ...
	    set -e
            met_pre_batch_to_cache RJTD wis_jma cache 1>> wis_jma_to_cache.txt 2>> met_pre_batch_to_cache.log
