...
if test ${is_pre} -eq 0; then
  aria2c -j 1 -s 1 -x 1 --header 'Cache-Control: no-cache' --auto-file-renaming=false --allow-overwrite=false --log-level=info -l wis_jma/aria2c.log -o wis_jma/created.txt "https://www.wis-jma.go.jp/data/syn?ContentType=Text&Access=Open" >> wis_jma/get_list_stdout.txt
else
  since="If-None-Match: ${etag}"
  aria2c -j 1 -s 1 -x 1 --header "${since}" --header 'Cache-Control: no-cache' --auto-file-renaming=false --allow-overwrite=false --log-level=info -l wis_jma/aria2c.log -o wis_jma/created.txt "https://www.wis-jma.go.jp/data/syn?ContentType=Text&Access=Open" >> wis_jma/get_list_stdout.txt
fi
...
if test ${err_num} -eq 0; then
  ...
  if test ${is_pre} -eq 1 -a -s wis_jma/created.txt; then
    ...
    while test ${created_num} -ne 0; do
      ...
      set +e
      aria2c -j ${parallel} -s ${parallel} -x ${parallel} --header 'Cache-Control: no-cache' --auto-file-renaming=false --allow-overwrite=false --log-level=error -l wis_jma/aria2c.log -i wis_jma/created.txt -d wis_jma/downloaded >> wis_jma/get_file_stdout.txt
      set -e
