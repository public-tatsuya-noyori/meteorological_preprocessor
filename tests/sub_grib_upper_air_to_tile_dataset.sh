      ...
      if test -s ${source_work_directory}/filtered_newly_created_file.tmp; then
        if test ${index_only} -eq 0; then
          ...
          if test -s ${work_directory}/processed_file.txt; then
            ...
            cp ${work_directory}/processed_file.txt ${work_directory}/${now}_${unique_center_id}.txt
            gzip -f ${work_directory}/${now}_${unique_center_id}.txt
            mv ${work_directory}/${now}_${unique_center_id}.txt.gz ${processed_directory}/
            sleep 1
            cat ${work_directory}/processed_file.txt | sed -e 's|\.gz||g' -e "s|^|${local_work_directory}/|g" > ${local_work_directory}/${process_prefix}.txt
            ./met_pre_grib_to_tile_dataset.py --debug ${local_work_directory}/${process_prefix}.txt ${local_work_directory} 1>${local_work_directory}/${process_prefix}/${now}.txt.tmp 2>>${local_work_directory}/${process_prefix}_stderr.log
            if test -s ${local_work_directory}/${process_prefix}/${now}.txt.tmp; then
              sort -u ${local_work_directory}/${process_prefix}/${now}.txt.tmp > ${local_work_directory}/${process_prefix}/${now}.txt
              cat ${local_work_directory}/${process_prefix}/${now}.txt | xargs -r -n 64 -P 16 gzip -f
              cat ${local_work_directory}/${process_prefix}/${now}.txt | xargs -r -n 1 -P 16 -I {} mv -f {}.gz {}
              sed -e "s|${local_work_directory}/||g" ${local_work_directory}/${process_prefix}/${now}.txt > ${local_work_directory}/${process_prefix}/${now}_raw.txt
              timeout -k 3 300 rclone copy --header-upload 'Content-Encoding: gzip' --checksum --contimeout 8s --files-from-raw ${local_work_directory}/${process_prefix}/${now}_raw.txt --log-file ${local_work_directory}/${process_prefix}/rclone.log --low-level-retries 3 --no-check-dest --no-traverse --retries 3 --s3-no-check-bucket --s3-no-head --stats 0 --timeout 8s --transfers 16 ${local_work_directory} ${dest_rclone_remote_bucket}
              gzip -f ${local_work_directory}/${process_prefix}/${now}_raw.txt
              timeout -k 3 50 rclone copyto --header-upload 'Content-Encoding: gzip' --checksum --contimeout 8s --log-file ${local_work_directory}/${process_prefix}/rclone.log --low-level-retries 3 --no-check-dest --no-traverse --retries 3 --s3-no-check-bucket --s3-no-head --stats 0 --timeout 8s --transfers 1 ${local_work_directory}/${process_prefix}/${now}_raw.txt.gz ${dest_rclone_remote_bucket}/4PubSub/${pubsub_child_directory}/${now}_${unique_center_id}.txt
              cat ${local_work_directory}/${process_prefix}/${now}.txt | xargs -r -n 1 -P 16 -I {} mv -f {} {}.gz
...
timeout=8s
dest_rclone_remote_bucket='minio:aa-open-dataset'
pubsub_child_directory=arrow
for arg in "$@"; do
  case "${arg}" in
    "--bandwidth_limit") bandwidth_limit_k_bytes_per_s=$2;shift;shift;;
    "--config") config=$2;shift;shift;;
    "--delete_index_minute" ) delete_index_minute=$2;shift;shift;;
    '--help' ) echo "$0 [--bandwidth_limit bandwidth_limit_k_bytes_per_s] [--config config_file] [--delete_index_minute delete_index_minute] [--index_only] [--no_check_pid] [--parallel number_of_parallel_transfer] [--timeout rclone_timeout] local_work_directory unique_center_id extension_type 'source_rclone_remote_bucket_main[;source_rclone_remote_bucket_sub]' inclusive_pattern_file exclusive_pattern_file"; exit 0;;
...
process_prefix=sub_grib_upper_air_to_tile_dataset
mkdir -p ${work_directory} ${processed_directory} ${local_work_directory}/${process_prefix}
if test -s ${work_directory}/${process_prefix}_pid.txt; then
  if test ${no_check_pid} -eq 0; then
    running=`cat ${work_directory}/${process_prefix}_pid.txt | xargs -r ps ho 'pid comm args' | grep -F " $0 " | grep -F " ${unique_center_id} " | grep -F " ${extension_type} " | wc -l`
  else
    exit 0
  fi
else
...
if test ${running} -eq 0; then
  subscribe &
  pid=$!
  echo ${pid} > ${work_directory}/${process_prefix}_pid.txt
  set +e
  wait ${pid}
  ec=$?
  set -e
  rm ${work_directory}/${process_prefix}_pid.txt
fi
exit ${ec}
