...
if test -s download_${priority}_${regions}${closed}/created.txt; then
  ...
  while test ${created_num} -gt 0; do
    ...
    ./met_pre_batch_to_cache.py ${center} download_${priority}_${regions}${closed}/downloaded cache download_${priority}_${regions}${closed}/checksum.feather 1>> download_${priority}_${regions}${closed}/cached/${now}.txt.tmp 2>> download_${priority}_${regions}${closed}/met_pre_batch_to_cache.log
