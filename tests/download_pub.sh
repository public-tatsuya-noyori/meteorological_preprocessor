...
for i in `ls -1 download_${priority}/cached/*|grep -v '\.tmp$'|uniq`;do ./pub.sh --cron --rm_input_index_file cache_o iij12oi ${priority} ${i} "${rclone_remote_bucket_list}" 64;done
} &
pid=$!
