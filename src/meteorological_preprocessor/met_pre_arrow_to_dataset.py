                ...
                for tile_y in range(0, 2**(zoom)):
                    ...
                    for new_datetime in new_datetime_list_dict[tile_x,  tile_y]:
                        ...
                        if len(new_df.index) > 0:
                            out_directory = ''.join([out_dir, '/', conf_tuple.convert, '/', conf_tuple.category, '/', conf_tuple.subcategory, '/', str(new_datetime.year).zfill(4), '/', str(new_datetime.month).zfill(2), str(new_datetime.day).zfill(2), '/', str(new_datetime.hour).zfill(2), str(math.floor(new_datetime.minute / 10)), '0/', str(zoom), '/', str(tile_x), '/', str(tile_y)])
                            ...
                            sort_unique_list.insert(0, 'indicator')
                            sort_unique_list.insert(1, 'time since created [s]')
                            new_df.sort_values(sort_unique_list, inplace=True)
                            sort_unique_list.remove('time since created [s]')
                            new_df.drop_duplicates(subset=sort_unique_list, keep='last', inplace=True)
                            sort_unique_list.insert(1, 'time since created [s]')
