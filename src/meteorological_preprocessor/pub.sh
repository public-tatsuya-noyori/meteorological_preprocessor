...
for arg in "$@"; do
  case "${arg}" in
    '--help' ) echo "$0 job_name raw_list_file local_dir dst_rclone_remote dst_bucket priority_name parallel access"; exit 0;;
  esac
done
if test $# -lt 8; then
...
if test ${is_running} -eq 0; then
  {
    ...
    grep "^${local_dir}/${access}/" ${raw_list_file} | sed -e "s|^${local_dir}/${access}/|/|g" > ${local_dir}/${access}/${job_dir}/${job_name}/${priority_name}/output_${job_datetime}.tmp
