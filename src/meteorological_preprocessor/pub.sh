...
if test -s ${local_dir}/${access}/4PubSub/${priority_name}/${pub_datetime}.txt; then
  ...
  while test ${unpub_num} -ne 0; do
    now=`date -u "+%Y%m%d%H%M%S"`
    set +e
    rclone --ignore-checksum --immutable --no-traverse --size-only --stats 0 --timeout 1m --s3-upload-cutoff 0 --log-level ERROR --log-file ${local_dir}/${access}/4Pub/${priority_name}/${pub_name}/log/${pub_datetime}_${now}_index.log copyto ${local_dir}/${access}/4PubSub/${priority_name}/${pub_datetime}.txt ${rclone_remote}:${bucket}/4PubSub/${priority_name}/${now}.txt
