    ...
    for conf_tuple in conf_df[(conf_df['category'] == cat) & (conf_df['subcategory'] == subcat)].itertuples():
        ...
        for tile_x in range(0, 2**(tile_level + 1)):
            for tile_y in range(0, 2**(tile_level)):
                ...
                new_datetime_list_dict[tile_x,  tile_y] = tile_df['datetime'].dt.floor('1T').unique()
                for new_datetime in new_datetime_list_dict[tile_x,  tile_y]:
                    ...
                    out_file = ''.join([out_dir, '/', cccc, '/bufr_to_arrow/', cat, '/', subcat, '/', str(new_datetime.year).zfill(4), '/', str(new_datetime.month).zfill(2), str(new_datetime.day).zfill(2), '/', str(new_datetime.hour).zfill(2), str(new_datetime.minute).zfill(2), '/', str(tile_level), '/', str(tile_x), '/', str(tile_y), '.arrow'])
