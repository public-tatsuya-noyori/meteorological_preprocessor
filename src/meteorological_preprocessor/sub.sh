        ...
        if test -s ${source_work_directory}/${pubsub_index_directory}_new_index.tmp; then
          ...
          if test -s ${source_work_directory}/${pubsub_index_directory}_index_diff.txt; then
            ...
            if test -s ${source_work_directory}/filtered_newly_created_file.tmp; then
              ...
              set +e
              rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --checkers ${parallel} --checksum --contimeout ${timeout} --cutoff-mode=cautious ${file_from_option} ${source_work_directory}/filtered_newly_created_file.tmp --immutable --local-no-set-modtime --log-file ${source_work_directory}/info_log.tmp --log-level DEBUG --low-level-retries 3 --multi-thread-cutoff ${cutoff} --multi-thread-streams ${parallel} --no-traverse --retries 3 --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${local_work_directory}
              exit_code=$?
              set -e
              ...
              set +e
              grep -E "^(.* DEBUG *: *[^ ]* *:.* Unchanged skipping.*|.* INFO *: *[^ ]* *:.* Copied .*)$" ${source_work_directory}/info_log.tmp | sed -e "s|^.* DEBUG *: *\([^ ]*\) *:.* Unchanged skipping.*$|/\1|g" -e "s|^.* INFO *: *\([^ ]*\) *:.* Copied .*$|/\1|g" | grep -v '^ *$' >> ${work_directory}/processed_file.txt
              set -e
            fi
          fi
        fi
