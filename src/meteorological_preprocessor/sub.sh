...
for priority_name in `rclone --stats 0 --timeout 1m --log-level ERROR --log-file ${local_dir}/${access}/4Sub_log/${sub_name}/${sub_datetime}.log lsf --max-depth 1 ${rclone_remote}:${bucket}/4PubSub | grep -E "${priority_name_pattern}" | grep -v '^ *$' | uniq`; do
