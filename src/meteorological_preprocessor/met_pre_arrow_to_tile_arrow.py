            ...
            for tile_x in range(0, 2**(zoom + 1)):
                for tile_y in range(0, 2**(zoom)):
                    ...
                    for new_datetime in new_datetime_list_dict[tile_x,  tile_y]:
                        unique_key_list = convert_cat_subcat_df['location_datetime'].values.tolist()
                        new_df = tile_df[(new_datetime <= tile_df['datetime']) & (tile_df['datetime'] < new_datetime + timedelta(minutes=10))].dropna(subset=unique_key_list).dropna(axis=1, how='all')
                        ...
                        if new_df_len > 0:
                            ...
                            unique_key_list.insert(0, 'indicator')
                            ...
                            old_df_index_list = old_df.index.tolist()
                            old_df_len = len(old_df_index_list)
                            if old_df_len > 0:
                                new_df_index_start = old_df_index_list[-1] + 1
                                new_df.index = range(new_df_index_start, new_df_index_start + new_df_len)
                                concat_df = pd.concat([old_df, new_df[unique_key_list]])
                                ...
                                keep_index_list = concat_df[duplicated].index.tolist()
                                out_file_dict[out_file] = concat_df.drop_duplicates(keep='last')
                                for column in new_df.columns:
                                    if not column in unique_key_list:
                                        new_column_df = new_df[[column]]
                                        old_column_df = new_column_df.iloc[0:0]
                                        out_file = ''.join([out_directory, '/', column.split('[')[0].strip(' ').replace(' ', '_'), '.feather'])
                                        if out_file in out_file_dict:
                                            old_column_df = out_file_dict[out_file]
                                        elif os.path.exists(out_file):
                                            old_column_df = feather.read_feather(out_file)
                                        old_column_df_len = len(old_column_df.index)
                                        if old_column_df_len > 0:
                                            concat_df = pd.concat([old_column_df, new_column_df])
                                        else:
                                            concat_df = new_column_df
                                        out_file_dict[out_file] = concat_df.loc[keep_index_list].dropna()
                            else:
                                out_file_dict[out_file] = new_df[unique_key_list]
                                for column in new_df.columns:
                                    if not column in unique_key_list:
                                        out_file = ''.join([out_directory, '/', column.split('[')[0].strip(' ').replace(' ', '_'), '.feather'])
                                        out_file_dict[out_file] = new_df[[column]].dropna()
