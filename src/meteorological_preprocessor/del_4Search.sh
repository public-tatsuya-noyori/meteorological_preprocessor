...
set -e
delete_4Search() {
  rclone lsf --contimeout ${timeout} --low-level-retries 3 --max-depth 1 --no-traverse --quiet --retries 1 --stats 0 --timeout ${timeout} ${rclone_remote_bucket}/${search_index_directory}/${priority}/ > ${source_work_directory}/${priority}_${search_index_directory}_date_hour_slash_directory.tmp
  if test -s ${source_work_directory}/${priority}_${search_index_directory}_date_hour_slash_directory.tmp; then
    rm -rf ${work_directory}/${search_index_directory}/${priority}
    for date_hour_directory in `cat ${source_work_directory}/${priority}_${search_index_directory}_date_hour_slash_directory.tmp | grep -v -E "^(${delete_index_date_hour_pattern})/$" | sed -e 's|/$||g'`; do
