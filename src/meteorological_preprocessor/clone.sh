...
clone() {
  ...
  for source_rclone_remote_bucket in `echo ${source_rclone_remote_bucket_main_sub} | tr ';' '\n'`; do
    ...
    rm -rf ${source_work_directory}/${pubsub_index_directory}/${extension}
    rm -rf ${source_work_directory}/${search_index_directory}/${extension}
    ...
    for get_pubsub_index_retry_count in `seq 2`; do
      ...
      if test ${index_only} -eq 0; then
        sed -e "s|^|/${pubsub_index_directory}/${extension}/|g" ${source_work_directory}/${pubsub_index_directory}_index_diff.txt > ${source_work_directory}/${pubsub_index_directory}_newly_created_index.tmp
      else
        grep "^[0-9]{14}_${index_only_center_id_prefix}.*\.txt\.gz$" ${source_work_directory}/${pubsub_index_directory}_index_diff.txt | sed -e "s|^|/${pubsub_index_directory}/${extension}/|g" > ${source_work_directory}/${pubsub_index_directory}_newly_created_index.tmp
      fi
      cp /dev/null ${source_work_directory}/err_log.tmp
      set +e
#      timeout -k 3 ${rclone_timeout} rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --checksum --contimeout ${timeout} --files-from-raw ${source_work_directory}/${pubsub_index_directory}_newly_created_index.tmp --local-no-set-modtime --log-file ${source_work_directory}/err_log.tmp --low-level-retries 3 --no-traverse --quiet --retries 3 --s3-no-check-bucket --s3-no-head --s3-no-head-object --azureblob-no-head-object --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${source_work_directory}
    ...
    if test ${exit_code} -ne 0; then
      cat ${source_work_directory}/err_log.tmp >&2
      echo "ERROR: ${exit_code}: can not get index file from ${source_rclone_remote_bucket}/${pubsub_index_directory}/${extension}." >&2
      continue
    fi
    if test -s ${source_work_directory}/${pubsub_index_directory}_index_diff.txt; then
      find ${source_work_directory}/${pubsub_index_directory}/${extension} -regextype posix-egrep -regex "^.*/[0-9]{14}_[^/]*\.txt.gz$" -type f | sed -e 's|.*/||g' > ${source_work_directory}/${pubsub_index_directory}_gotten_new_index.tmp
      set +e
      ...
      if test ${cmp_exit_code_1} -eq 1 -o ${cmp_exit_code_2} -eq 0; then
        cp /dev/null ${source_work_directory}/err_log.tmp
        set +e
        timeout -k 3 ${rclone_timeout} rclone lsf --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --contimeout ${timeout} --log-file ${source_work_directory}/err_log.tmp --low-level-retries 3 --max-depth 1 --no-traverse --quiet --retries 3 --stats 0 --timeout ${timeout} ${source_rclone_remote_bucket}/${search_index_directory}/${extension}/ > ${source_work_directory}/${search_index_directory}_date_hour_slash_directory.tmp
        exit_code=$?
        set -e
        if test ${exit_code} -ne 0; then
          cat ${source_work_directory}/err_log.tmp >&2
          echo "ERROR: ${exit_code}: can not get index directory list from ${source_rclone_remote_bucket}/${search_index_directory}/${extension}." >&2
          continue
        fi
        ...
        if test -s ${source_work_directory}/${search_index_directory}_date_hour_directory.tmp; then
          ...
          for date_hour_directory in `tac ${source_work_directory}/${search_index_directory}_date_hour_directory.tmp`; do
            ...
            if test ${exit_code} -ne 0; then
              ...
              cat ${source_work_directory}/err_log.tmp >&2
              echo "ERROR: ${exit_code}: can not get index file list from ${source_rclone_remote_bucket}/${search_index_directory}/${extension}/${date_hour_directory}." >&2
              break
            fi
            if test ${index_only} -eq 0; then
...
#          timeout -k 3 ${rclone_timeout} rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --checksum --contimeout ${timeout} --files-from-raw ${source_work_directory}/${search_index_directory}_newly_created_index.tmp --local-no-set-modtime --log-file ${source_work_directory}/err_log.tmp --low-level-retries 3 --no-traverse --quiet --retries 3 --s3-no-check-bucket --s3-no-head --s3-no-head-object --azureblob-no-head-object --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${source_work_directory}
          ...
          if test ${exit_code} -ne 0; then
            cat ${source_work_directory}/err_log.tmp >&2
            echo "ERROR: ${exit_code}: can not get index file from ${source_rclone_remote_bucket}/${search_index_directory}/${extension}." >&2
            continue
          fi
        fi
      fi
...
#            timeout -k 3 ${rclone_timeout} rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --checksum --contimeout ${timeout} --files-from-raw ${source_work_directory}/filtered_newly_created_file.tmp --log-file ${source_work_directory}/info_log.tmp --log-level DEBUG --low-level-retries 3 --no-check-dest --no-traverse --retries 3 --s3-no-check-bucket --s3-no-head --s3-no-head-object --azureblob-no-head-object --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${destination_rclone_remote_bucket}
            ...
            set +e
#            timeout -k 3 ${rclone_timeout} rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --checksum --contimeout ${timeout} --files-from-raw ${source_work_directory}/filtered_newly_created_file.tmp --header-upload "${header_upload}" --log-file ${source_work_directory}/info_log.tmp --log-level DEBUG --low-level-retries 3 --no-check-dest --no-traverse --retries 3 --s3-no-check-bucket --s3-no-head --s3-no-head-object --azureblob-no-head-object --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${destination_rclone_remote_bucket}
            timeout -k 3 ${rclone_timeout} rclone copy --bwlimit ${bandwidth_limit_k_bytes_per_s} --config ${config} --checksum --contimeout ${timeout} --files-from-raw ${source_work_directory}/filtered_newly_created_file.tmp --header-upload "${header_upload}" --log-file ${source_work_directory}/info_log.tmp --log-level DEBUG --low-level-retries 3 --no-check-dest --no-traverse --retries 3 --s3-no-check-bucket --s3-no-head --s3-no-head-object --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${destination_rclone_remote_bucket}
            exit_code=$?
            set -e
          fi
          if test ${exit_code} -ne 0; then
            if test ${exit_code} -eq 124; then
              ...
              if test -s ${source_work_directory}/rclone_timeout.txt; then
                echo "ERROR: rclone timeout ${exit_code}: terminated clone file from ${source_rclone_remote_bucket} ${extension} to ${destination_rclone_remote_bucket} ${extension}." >&2
                rm -f ${source_work_directory}/${pubsub_index_directory}_index.txt
              ...
              set -e
              echo "ERROR: ${exit_code}: can not clone file from ${source_rclone_remote_bucket} ${extension} to ${destination_rclone_remote_bucket} ${extension}." >&2
            fi
            continue
          fi
          cp /dev/null ${source_work_directory}/rclone_timeout.txt
          ...
          if test -s ${work_directory}/processed_file.txt; then
            ...
            if test ${exit_code} -ne 0; then
              cat ${work_directory}/err_log.tmp >&2
              echo "ERROR: ${exit_code}: can not put ${now}.txt on ${destination_rclone_remote_bucket}/${pubsub_index_directory}/${extension}/." >&2
            fi
          fi
        fi
      fi
