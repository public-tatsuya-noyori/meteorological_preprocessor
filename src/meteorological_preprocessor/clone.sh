...
clone() {
  ...
  while test ${job_count} -le ${job_num}; do
    ...
    for source_rclone_remote_bucket in `echo ${source_rclone_remote_bucket_list} | tr ';' '\n'`; do
      ...
      if test ! -f ${source_work_directory}/${priority}_index.txt; then
        if test ${sub} -eq 1; then
          cp /dev/null ${source_work_directory}/${priority}_index.txt
        else
          set +e
          rclone lsf --contimeout ${timeout} --low-level-retries 3 --max-depth 1 --no-traverse --quiet --retries 1 --stats 0 --timeout ${timeout} ${source_rclone_remote_bucket}/${pubsub_index_directory}/${priority} > ${source_work_directory}/${priority}_index.txt
          tmp_exit_code=$?
          set -e
          if test ${tmp_exit_code} -ne 0; then
            ...
            rm -f ${source_work_directory}/${priority}_index.txt
            continue
          fi
        fi
      fi
      ...
      if test -s ${source_work_directory}/${priority}_${pubsub_index_directory}_new_index.tmp; then
        diff ${source_work_directory}/${priority}_index.txt ${source_work_directory}/${priority}_${pubsub_index_directory}_new_index.tmp | grep '>' | cut -c3- > ${source_work_directory}/${priority}_index_diff.tmp
        if test -s ${source_work_directory}/${priority}_index_diff.tmp; then
          sed -e "s|^|/${pubsub_index_directory}/${priority}/|g" ${source_work_directory}/${priority}_index_diff.tmp > ${source_work_directory}/${priority}_${pubsub_index_directory}_newly_created_index.tmp
          set +e
          ...
          set +e
          cmp -s ${source_work_directory}/${priority}_index_diff.tmp ${source_work_directory}/${priority}_${pubsub_index_directory}_gotten_new_index.tmp
          cmp_exit_code_1=$?
          set -e
          ...
          set +e
          cmp -s ${source_work_directory}/${priority}_index_diff.tmp ${source_work_directory}/${priority}_${pubsub_index_directory}_new_index.tmp
          cmp_exit_code_2=$?
          set -e
          ...
          if test ${cmp_exit_code_1} -eq 1 -o ${cmp_exit_code_2} -eq 0; then
            ...
            if test -s ${source_work_directory}/${priority}_${search_index_directory}_date_hour_directory.tmp; then
              former_index_file_first_line=`head -1 ${source_work_directory}/${priority}_index.txt`
              for date_hour_directory in `tac ${source_work_directory}/${priority}_${search_index_directory}_date_hour_directory.tmp`; do
                ...
                former_index_file_first_line_count=`grep ${source_work_directory}/${priority}_${search_index_directory}_index.tmp | wc -l`
                if test ${former_index_file_first_line_count} -eq 0; then
                  grep -v -f ${source_work_directory}/${priority}_${pubsub_index_directory}_gotten_new_index.tmp ${source_work_directory}/${priority}_${search_index_directory}_index.tmp >> ${source_work_directory}/${priority}_${search_index_directory}_new_index.tmp
                  continue
                else
                  sed -ne "/${former_index_file_first_line}/,\$p" ${source_work_directory}/${priority}_${search_index_directory}_index.tmp | grep -v -f ${source_work_directory}/${priority}_${pubsub_index_directory}_gotten_new_index.tmp >> ${source_work_directory}/${priority}_${search_index_directory}_new_index.tmp
                fi
              done
          ...
          if test -s ${source_work_directory}/${priority}_filtered_newly_created_file.tmp; then
            ...
            if test ${wildcard_index} -eq 1; then
              rclone copy --checkers ${parallel} --contimeout ${timeout} --cutoff-mode=cautious --ignore-checksum --include-from ${source_work_directory}/${priority}_filtered_newly_created_file.tmp --low-level-retries 3 --multi-thread-cutoff ${cutoff} --multi-thread-streams ${parallel} --no-check-dest --no-traverse --quiet --retries 1 --s3-chunk-size ${cutoff} --s3-upload-concurrency ${parallel} --size-only --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${destination_rclone_remote_bucket}
            else
              rclone copy --checkers ${parallel} --contimeout ${timeout} --cutoff-mode=cautious --files-from-raw ${source_work_directory}/${priority}_filtered_newly_created_file.tmp --ignore-checksum --low-level-retries 3 --multi-thread-cutoff ${cutoff} --multi-thread-streams ${parallel} --no-check-dest --no-traverse --quiet --retries 1 --s3-chunk-size ${cutoff} --s3-upload-concurrency ${parallel} --size-only --stats 0 --timeout ${timeout} --transfers ${parallel} ${source_rclone_remote_bucket} ${destination_rclone_remote_bucket}
            fi
            tmp_exit_code=$?
            set -e
            if test ${tmp_exit_code} -ne 0; then
    ...
    if test -s ${work_directory}/${priority}_merged_filtered_newly_created_file.tmp; then
      tmp_exit_code=1
      retry_count=1
      ...
      while test ${tmp_exit_code} -ne 0; do
        ...
        set +e
        rclone copyto --contimeout ${timeout} --ignore-checksum --immutable --log-file ${work_directory}/${priority}_log.tmp --low-level-retries 3 --no-traverse --quiet --retries 1 --size-only --stats 0 --timeout ${timeout} ${work_directory}/${priority}_merged_filtered_newly_created_file.tmp ${destination_rclone_remote_bucket}/${pubsub_index_directory}/${priority}/${now}.txt
        tmp_exit_code=$?
        set -e
