...
def convert_to_arrow(my_cccc, in_file_list, out_dir, cat, subcat, out_list_file, conf_location_datetime_list, conf_property_list, debug):
    warno = 189
    out_arrows = []
    now = datetime.utcnow()
    ...
    for cccc in list(set([re.sub('^.*/', '', re.sub('/bufr/' + cat + '/' + subcat + '/' +'.*$', '', in_file)) for in_file in in_file_list if re.match(r'^.*/bufr/' + cat + '/' + subcat + '/.*$', in_file)])):
                    ...
                    try:
                        ...
                        for conf_row in conf_location_datetime_list:
                            ...
                            if len(values) > 0:
                                ...
                                tmp_location_datetime_dict[conf_row.key] = values
                                ...
                                is_location_datetime = False
                                break
                        if not is_location_datetime or not True in none_np.tolist():
                            codes_release(bufr)
                            break
                        tmp_location_datetime_dict['none'] = none_np
                    except CodesInternalError as err:
                        print('Warning', warno, ':', 'CodesInternalError is happend at tmp_location_datetime_dict in', in_file, file=sys.stderr)
                        break
                    tmp_property_dict = {}
                    try:
                        for conf_row in conf_property_list:
                            ...
                            if len(values) > 0:
                                if values[0] == str:
                                    ...
                                    values = np.where(np.isnan(values), None, values)
                                    values = np.where((values < conf_row.min) | (values > conf_row.max), None, values)
                                tmp_property_dict[conf_row.name] = values
                    except CodesInternalError as err:
                        print('Warning', warno, ':', 'CodesInternalError is happend at tmp_property_dict in', in_file, file=sys.stderr)
                        break
                    codes_release(bufr)
                    if len(tmp_property_dict) == 0 or len(tmp_location_datetime_dict) == 0:
                        break
                    else:
                        location_datetime_index_np = np.array([index for index, value in enumerate(tmp_location_datetime_dict['none']) if value == True])
                        ...
                        conf_location_datetime_name_keys_dict = {}
                        for conf_row in conf_location_datetime_list:
                            datatype_dict[conf_row.name] = conf_row.datatype
                            if conf_row.name in conf_location_datetime_name_keys_dict:
                                conf_location_datetime_name_keys_dict[conf_row.name] = conf_location_datetime_name_keys_dict[conf_row.name] + [conf_row.key]
                            else:
                                conf_location_datetime_name_keys_dict[conf_row.name] = [conf_row.key]
                            if conf_row.name == 'location':
                                tmp_loc_id_np = tmp_location_datetime_dict[conf_row.key]
                                if len(location_datetime_index_np) > 0:
                                    tmp_loc_id_np = tmp_loc_id_np[location_datetime_index_np]
                                ...
                                tmp_datetime_np = tmp_location_datetime_dict[conf_row.key]
                                if len(location_datetime_index_np) > 0:
                                    tmp_datetime_np = tmp_datetime_np[location_datetime_index_np]
                                ...
                                tmp_latitude_np = tmp_location_datetime_dict[conf_row.key]
                                if len(location_datetime_index_np) > 0:
                                    tmp_latitude_np = tmp_latitude_np[location_datetime_index_np]
                                ...
                                tmp_longitude_np = tmp_location_datetime_dict[conf_row.key]
                                if len(location_datetime_index_np) > 0:
                                    tmp_longitude_np = tmp_longitude_np[location_datetime_index_np]
                                ...
                                tmp_height_np = tmp_location_datetime_dict[conf_row.key]
                                if len(location_datetime_index_np) > 0:
                                    tmp_height_np = tmp_height_np[location_datetime_index_np]
                        ...
                        if len(message_loc_id_np) > 0 and len(message_datetime_np) > 0:
                            if 'location' in location_datetime_dict:
                                location_datetime_dict['location'] = np.concatenate([location_datetime_dict['location'], message_loc_id_np])
                            elif 'location' in conf_location_datetime_name_keys_dict.keys():
                                location_datetime_dict['location'] = message_loc_id_np
                            tmp_datetime_list = []
                            ...
                            if 'datetime' in location_datetime_dict:
                                location_datetime_dict['datetime'] = location_datetime_dict['datetime'] + tmp_datetime_list
                            elif 'datetime' in conf_location_datetime_name_keys_dict.keys():
                                location_datetime_dict['datetime'] = tmp_datetime_list
                            if 'latitude [degree]' in location_datetime_dict:
                                location_datetime_dict['latitude [degree]'] = np.concatenate([location_datetime_dict['latitude [degree]'], message_latitude_np])
                            elif 'latitude [degree]' in conf_location_datetime_name_keys_dict.keys():
                                location_datetime_dict['latitude [degree]'] = message_latitude_np
                            if 'longitude [degree]' in location_datetime_dict:
                                location_datetime_dict['longitude [degree]'] = np.concatenate([location_datetime_dict['longitude [degree]'], message_longitude_np])
                            elif 'longitude [degree]' in conf_location_datetime_name_keys_dict.keys():
                                location_datetime_dict['longitude [degree]'] = message_longitude_np
                            if 'height' in location_datetime_dict:
                                location_datetime_dict['height'] = np.concatenate([location_datetime_dict['height'], message_height_np])
                            elif 'height' in conf_location_datetime_name_keys_dict.keys():
                                location_datetime_dict['height'] = message_height_np
                            for conf_row in conf_property_list:
                                datatype_dict[conf_row.name] = conf_row.datatype
                                if conf_row.name in tmp_property_dict:
                                    tmp_property_np = tmp_property_dict[conf_row.name]
                                    if len(location_datetime_index_np) > 0:
                                        tmp_property_np = tmp_property_np[location_datetime_index_np]
                                    if len(tmp_property_np) > 0:
                                        if conf_row.name in property_dict:
                                            property_dict[conf_row.name] = np.concatenate([property_dict[conf_row.name], tmp_property_np])
                                        else:
        ...
        if 'datetime' in location_datetime_dict:
            ...
            for key in location_datetime_dict.keys():
                if key == 'datetime':
                    location_datetime_data.append(pa.array(location_datetime_dict[key], pa.timestamp('ms', tz='utc')))
                else:
                    location_datetime_data.append(pa.array(location_datetime_dict[key].flatten(), datatype_dict[key]))
                name_list.append(key)
            datetime_directory_list = [str(datetime.year).zfill(4) + str(datetime.month).zfill(2) + str(datetime.day).zfill(2) + str(datetime.hour).zfill(2) + str(datetime.minute).zfill(2)[0:1] + "0" for datetime in location_datetime_dict['datetime']]
            for datetime_directory in set(datetime_directory_list):
                datetime_index_list = [index for index, value in enumerate(location_datetime_dict['datetime']) if value.year == int(datetime_directory[0:4]) and value.month == int(datetime_directory[4:6]) and value.day == int(datetime_directory[6:8]) and value.hour == int(datetime_directory[8:10]) and value.minute / 10 == int(datetime_directory[10:11])]
...
def main():
    ...
    config_location_datetime = pkg_resources.resource_filename(__name__, 'conf_bufr_to_arrow_' + args.category + '_' + args.subcategory + '_location_datetime.csv')
    config_property = pkg_resources.resource_filename(__name__, 'conf_bufr_to_arrow_' + args.category + '_' + args.subcategory + '_property.csv')
    ...
    if not os.access(config_location_datetime, os.F_OK):
        print('Error', errno, ':', config_location_datetime, 'does not exist.', file=sys.stderr)
        sys.exit(errno)
    if not os.access(config_property, os.F_OK):
        print('Error', errno, ':', config_property, 'does not exist.', file=sys.stderr)
        sys.exit(errno)
    ...
    if not os.path.isdir(args.output_directory):
        ...
        sys.exit(errno)
    if not os.path.isfile(config_location_datetime):
        print('Error', errno, ':', config_location_datetime, 'is not file.', file=sys.stderr)
        sys.exit(errno)
    if not os.path.isfile(config_property):
        print('Error', errno, ':', config_property, 'is not file.', file=sys.stderr)
        sys.exit(errno)
    ...
    if not (os.access(args.output_directory, os.R_OK) and os.access(args.output_directory, os.W_OK) and os.access(args.output_directory, os.X_OK)):
        ...
        sys.exit(errno)
    if not os.access(config_location_datetime, os.R_OK):
        print('Error', errno, ':', config_location_datetime, 'is not readable.', file=sys.stderr)
        sys.exit(errno)
    if not os.access(config_property, os.R_OK):
        print('Error', errno, ':', config_property, 'is not readable.', file=sys.stderr)
        sys.exit(errno)
    try:
        input_file_list = []
        ...
        conf_location_datetime_list = list(csv.read_csv(config_location_datetime).to_pandas().itertuples())
        conf_property_list = list(csv.read_csv(config_property).to_pandas().itertuples())
        convert_to_arrow(args.my_cccc, input_file_list, args.output_directory, args.category, args.subcategory, args.output_list_file, conf_location_datetime_list, conf_property_list, args.debug)
    except:
